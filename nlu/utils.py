...
def prepare_classification_features(document: str, candidates: List[str], tokenizer) -> List[Dict[str, Any]]:
...
def postprocess_classification_predictions(predictions: np.ndarray, entail_idx: int = 0) -> Dict[str, Any]:
    """
...
def prepare_extraction_features(examples: List[Any], tokenizer) -> List[Dict[str, Any]]:
    # Some of the questions have lots of whitespace on the left, which is not useful and will make the
    # truncation of the context fail (the tokenized question will take a lots of space). So we remove that
    # left whitespace
    question = [example["question"].lstrip() for example in examples]
...
def postprocess_extraction_predictions(
    examples,
    features,
